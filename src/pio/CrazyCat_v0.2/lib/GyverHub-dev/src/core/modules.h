...
struct Modules {
    ...
    bool read(Module m) {
        return !(mods & (uint32_t)m);
    }
    bool read(uint32_t m) {
        return !(mods & (uint32_t)m);
    }

    bool checkConnection(gh::Connection conn) {
        return !((!read(gh::Module::ModSerial) && conn == gh::Connection::Serial) ||
                 (!read(gh::Module::ModBluetooth) && conn == gh::Connection::Bluetooth) ||
                 (!read(gh::Module::ModWS) && conn == gh::Connection::WS) ||
                 (!read(gh::Module::ModHTTP) && conn == gh::Connection::HTTP) ||
                 (!read(gh::Module::ModMQTT) && conn == gh::Connection::MQTT));
    }
...
#ifndef GH_NO_FS
            case gh::CMD::FsAbort:
                return read(gh::Module::ModFetch | gh::Module::ModUpload | gh::Module::ModOta);
            case gh::CMD::Files:
