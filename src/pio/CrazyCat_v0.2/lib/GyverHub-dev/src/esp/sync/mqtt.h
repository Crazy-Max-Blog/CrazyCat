...
class MQTT {
   ...
   public:
    ...
    void begin() {
        mqtt.setCallback([this](char* topic, uint8_t* data, uint16_t len) {
            ...
            parse_h(hubptr, topic_buf, data_buf, gh::Connection::MQTT, gh::Source::ESP);
        });
    }
   ...
   private:
    ...
    void setup(void* hub, ParseHook prh, char* id, String* prefix) {
        this->hubptr = hub;
        this->parse_h = prh;
        this->id = id;
        this->prefix = prefix;
    ...
    void send(const char* topic, gh::core::AnyText& text) {
        if (!mqtt.connected()) return;
        mqtt.beginPublish(topic, text.len, ret);
        if (text.pgm) {
            uint8_t buf[text.len];
            memcpy_P(buf, text.str, text.len);
            mqtt.write(buf, text.len);
        } else {
            mqtt.write((uint8_t*)text.str, text.len);
        }
        mqtt.endPublish();
    }
