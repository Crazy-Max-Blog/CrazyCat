...
// not littlefs
    ...
    // remove
    bool remove(PGM_P path, bool remdir = true) {
    ...
    // rename
    bool rename(PGM_P from, PGM_P to) {
    ...
    void showFiles(String& answ, const String& path, GH_UNUSED uint8_t levels = 0, uint32_t* count = nullptr) {
        ...
        while (dir.next()) {
            if (dir.isDirectory()) {
                ...
                gh::core::HubFS::showFiles(answ, p);
            }
            if (dir.isFile() && dir.fileName().length()) {
                answ += '\"';
                answ += path;
        ...
        while (file = root.openNextFile()) {
            if (file.isDirectory()) {
                ...
                if (levels) gh::core::HubFS::showFiles(answ, file.path(), levels - 1);
            } else {
                answ += '\"';
