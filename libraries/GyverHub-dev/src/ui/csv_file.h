...
class CSVFile {
   public:
    ...
    CSVFile& addStr(gh::core::AnyText text) {
        if (_col < _cols && _opened()) {
            _checkFirst();
            if (text.pgm) _file.print((const __FlashStringHelper*)text.str);
            else _file.print(text.str);
            _col++;
        }
        return *this;
    }
    ...
    // добавить float
    CSVFile& addFloat(float val, uint8_t dec = 2) {
        if (_col < _cols && _opened()) {
            _checkFirst();
            _file.print(val, dec);
            _col++;
        ...
        if (_opened()) {
            ...
            if (_rows && _row > _rows) {
                ...
                if (_file) {
                    ...
                    String npath(_path);
                    npath += F(".new");
                    File file = gh::FS.openWrite(npath);
                    if (file) {
                        file.write(_file);
                        file.close();
                        _file.close();
                        if (gh::FS.remove(_path, false) && gh::FS.rename(npath, _path)) {
                            _row = row;
                        }
                    }
                }
   ...
   private:
    ...
    bool _opened() {
        if (!_updated) update();
        if (!_file) _file = gh::FS.openAdd(_path);
        return _file;
    }
    void _checkFirst() {
