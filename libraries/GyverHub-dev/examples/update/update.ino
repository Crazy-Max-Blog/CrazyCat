...
void build(gh::Builder& b) {
    if (b.beginRow()) {
        ...
        b.Input_("inp").size(3).valueStr("hello").color(gh::Colors::Blue);

        // обновлять будем например по нажатию на кнопку. Кнопки для значения и цвета
        if (b.Button().size(1).label("value").click()) hub.update("inp").valueInt(random(1000));
        ...
        // hub.update("inp").valueInt(random(1000));  // это не будет работать
        b.endRow();
    }
    ...
    if (b.beginRow()) {
        ...
        if (b.Button().size(2).label("update").click()) {
            hub.update("sld").valueInt(random(100)).color(gh::Colors::Pink).unit("°");
            ...
            // hub.update("sld", &b.build.client).valueInt(random(100)).color(gh::Colors::Pink).unit("°");
        }
        b.endRow();
    }
...
void loop() {
    ...
    if (tmr) {
        hub.update("tit").valueInt(millis());
    }
    ...
    if (tmr2) {
        ...
        upd.update("name1").valueInt(random(100));
